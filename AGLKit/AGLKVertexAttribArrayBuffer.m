   ...
   usage:(GLenum)usage;
{
   ...
   if(nil != (self = [super init]))
   {
      ...
      glGenBuffers(1,                // STEP 1  //标识符被生成,并保存在 name 实例变量中
         &name);
      glBindBuffer(GL_ARRAY_BUFFER,  // STEP 2  //绑定用于指定标识符的缓存到当前缓存
         self.name); 
      glBufferData(                  // STEP 3
         GL_ARRAY_BUFFER,  // Initialize buffer contents  用于指定要更新当前上下文中所绑定的是哪一个缓 存
         bufferSizeBytes,  // Number of bytes to copy     要复制进这个缓存的字节的数量
         dataPtr,          // Address of bytes to copy    要复制的字节的地 址
         usage);           // Hint: cache in GPU memory   提示了缓存在未来的运算中可能将会被怎样使用
