   ...
   error:(NSError **)outError; 
{
   ...
   glTexImage2D(                                        // Step 3
      GL_TEXTURE_2D,    //第一个 数是用于 2D 纹理的 GL_ TEXTURE_2D
      0,                //第二个 数用于指定 MIP 贴图的初始细节级别。如果没有使用 MIP 贴 图,第2个参数必 是 0。如果开启了 MIP 贴图,使用第 个 数来明确地初始化每个 细节级别,但是要小心,因为从全分 率到只有一纹 的每个级别都必  指定, 则 GPU 将不会接 这个纹理缓存。
      GL_RGBA,          //第三个 数是 internalFormat,用于指定在纹理缓存内每个纹 需要 保存的信息的数量。
      (GLuint)width,    //第四个和第 个 数用于指定图像的 宽度和高度  高度和宽度需要是 2 的幂
      (GLuint)height,
      0,                //border  参数一直是用来确定围绕纹理的纹素的一个边界的大小,但是在 OpenGL ES 中它总是被设置为 0。
      GL_RGBA,          //第 个 数 format 用于指定初始化缓存所使用的图像 数据中的每个像 所要保存的信息,这个 数应该总是与 internalFormat  数相同。其他的 OpenGL 版本可能在 format 和 internalFormat  数不一 时自动 行图像数据格式的 换。
      GL_UNSIGNED_BYTE, //用于指定缓存中的纹素数据所使用的位编码类型
      [imageData bytes]);//是一个要 复制到指定的纹理缓存中的图片的像素颜色数据的指针。
